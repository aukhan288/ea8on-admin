...
public function store(Request $request)
{
    ...
    if ($request->hasFile('thumbnail')) {
        ...
        if ($attachment->isValid()) {
            ...
            $subcategory = SubCategory::create([
                ...
                'name' => $request->name,  // Adjust according to your needs
                'thumbnail' => $fileUrl  // Store the full URL to access the file
            ]);
    ...
    {
        $categories = SubCategory::where('category_id',$category)->get(['id', 'name']);
        return response()->json(['categories' => $categories]);
    }
}
